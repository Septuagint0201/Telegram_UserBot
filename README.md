...
M1候选在Windows/CPython 3.14.7的当前本地结果：76 tests `PASS`、10个integration/recovery test因本机无Docker按默认marker未运行，总coverage 98.07%；Ruff、strict mypy、import boundary、wheel/sdist Disclosure与secret/artifact扫描均`PASS`。这不是M1关闭证据；真实PostgreSQL/Redis、migration round trip、role、recovery与EXPLAIN必须由候选GitLab job实际通过。
