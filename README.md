...
M7新增deterministic occurrence/candidate、15分钟补偿扫描、DST/quiet/absolute no-send、account/contact/bypass reservation、strict Proactive Agent decision、Main AI text-only proactive context、AUTO/COPILOT final gate和send-unknown保守结算。quiet默认`22:00–08:00`，absolute no-send固定`00:00–07:00`。Windows/CPython 3.14.7 的 M7 unit tests、Ruff、strict mypy、import boundary、build、Disclosure和artifact static checks为`PASS`；签名提交`b738a9c3b1b9bef6da547be81eb6ddf383407cde`、source tree `90c45ec00c8e349b8b65f13108fae2d26cc710e1`对应的[GitHub Actions #31942739207](https://github.com/Septuagint0201/Telegram_UserBot/actions/runs/31942739207)中Preflight、PostgreSQL/Redis integration和Chromium browser contract全部为`PASS`，Preflight执行346 tests且0 failures，integration执行393 tests且0 failures，line/branch coverage为92.25%/84.22%，migration head为`0014_m7_budget_integrity`、SHA256为`a6154211b324a19c4c73de60e555c61b771009825eca0c3e4aa1cb6278679c71`，migration paths与M7 acceptance manifest均为`PASS`。本机无Docker，Windows本地PostgreSQL/Redis service test为`NOT RUN`；真实Telegram/provider、Control Bot polling、真实AUTO、部署、backup/restore与production load保持`NOT RUN`。
