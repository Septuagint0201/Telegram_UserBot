...
def upgrade() -> None:
    ...
    op.execute(
        ...
        "authorization_generation = candidate.generation, "
        "account_local_date = reservation.local_date, "
        "contact_local_date = reservation.local_date, "
        "held_at = COALESCE(reservation.held_at, reservation.created_at), "
        "account_bucket_id = account_bucket.id, "
        "contact_bucket_id = contact_bucket.id, "
        "bypass_bucket_id = CASE WHEN reservation.bypass THEN bypass_bucket.id ELSE NULL END "
        "FROM proactive_candidates AS candidate "
        "JOIN proactive_decisions AS decision ON decision.candidate_id = candidate.id "
        "AND decision.account_id = candidate.account_id "
        "JOIN proactive_budget_buckets AS account_bucket "
        "ON account_bucket.account_id = reservation.account_id "
        "AND account_bucket.contact_id IS NULL AND account_bucket.scope = 'account_daily' "
        "AND account_bucket.local_date = reservation.local_date "
        "JOIN proactive_budget_buckets AS contact_bucket "
        "ON contact_bucket.account_id = reservation.account_id "
        "AND contact_bucket.contact_id = reservation.contact_id "
        "AND contact_bucket.scope = 'contact_daily' "
        "AND contact_bucket.local_date = reservation.local_date "
        "LEFT JOIN proactive_budget_buckets AS bypass_bucket "
        "ON bypass_bucket.account_id = reservation.account_id "
        "AND bypass_bucket.contact_id = reservation.contact_id "
        "AND bypass_bucket.scope = 'contact_bypass' "
        "AND bypass_bucket.local_date = reservation.local_date "
        "WHERE reservation.candidate_id = candidate.id "
        "AND reservation.account_id = candidate.account_id "
        "AND (reservation.conversation_id IS NULL OR reservation.decision_id IS NULL "
        "OR reservation.account_bucket_id IS NULL OR reservation.contact_bucket_id IS NULL)"
    )
    op.execute(
