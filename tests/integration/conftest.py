...
def migrated_database(postgres_dsn: str) -> Iterator[str]:
    ...
    try:
        ...
        command.upgrade(config, "head")
        command.downgrade(config, "0007_m6_memory_pipeline")
        command.upgrade(config, "head")
        command.downgrade(config, "0008_m7_proactive_pipeline")
        command.upgrade(config, "head")
        command.downgrade(config, "0009_m5_m6_account_scope")
        command.upgrade(config, "head")
        command.downgrade(config, "0010_account_scope_refs")
        command.upgrade(config, "head")
